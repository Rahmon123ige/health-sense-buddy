...
const Chat: React.FC = () => {
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      type: "ai",
      content:
        "Hello! I'm your AI health assistant. Please describe your symptoms and I'll help you understand what might be going on and how urgent it might be. Remember, I'm here to provide guidance, not replace professional medical advice.",
      timestamp: new Date(),
    },
  ]);
  ...
  const analyzeSymptoms = async (
    symptoms: string
  ): Promise<{ analysis: string; urgency: "low" | "medium" | "high" }> => {
    // Replace with actual OpenAI API call
    const API_KEY =
      "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";
...
Be warm, understanding, and supportive. Remember many users may have limited healthcare access.`,
      ...
      if (
        ...
        analysis.toLowerCase().includes("low urgency")
      ) {
        urgency = "low";
      } else if (
        analysis.includes("HIGH") ||
        analysis.toLowerCase().includes("high urgency") ||
      ...
      return {
        analysis:
          "I'm sorry, I'm having trouble connecting to my analysis system right now. Please try again in a moment, or consult with a healthcare professional if your symptoms are concerning.",
        urgency: "medium",
      };
    }
  };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const existingHistory = JSON.parse(
        localStorage.getItem("ai-welfare-chat-history") || "[]"
      );
      const chatSession = {
        id: Date.now().toString(),
      ...
      const errorMessage: Message = {
        ...
        type: "ai",
        content:
          "I'm sorry, I encountered an error while analyzing your symptoms. Please try again or consult with a healthcare professional.",
        timestamp: new Date(),
      };
